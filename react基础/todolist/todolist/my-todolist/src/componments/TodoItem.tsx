...
const style2={
    ...
    position: 'absolute',
    left:'450px',
    background: 'url(C:\Users\lenovo\Desktop\react基础\jsx\image\删除.png)'
}as React.CSSProperties
const styleLink={
    position: 'absolute',
    left:'310px',
...
const [S,setS]=useState(["未开始","未完成","已完成"])
    ...
    return(
        ...
        <span style={idStyle}>{todo.id}</span>
            ...
            <span><select  style={style1}>
  <option >未开始</option>
  <option >待完成</option>
  <option >已完成</option>
</select ></span>
<span><button style={style2} >删除</button></span>
<Link style={styleLink}to={'./detail?id=123'}>详情</Link>
<p>截止时间:{todo.text1}年{todo.text2}月{todo.text3}日{todo.text4}时{todo.text5}分</p>
